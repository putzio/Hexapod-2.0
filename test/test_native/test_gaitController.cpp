...
namespace logic {
    ...
    TEST(GaitController, test_Default_Tripod) {
        GaitController gaitController = logic::GaitController();
        ASSERT_EQ(RESULT_DIRECTION_NOT_CHOSEN, gaitController.ChangeGait(logic::gait::TRIPOD));
        ASSERT_EQ(RESULT_OK, gaitController.ChangeDirection(logic::GaitController::DEFAULT_POSITION));
        std::array<bool, 6> groundDetected;
        ASSERT_EQ(RESULT_OK, gaitController.PeriodicProcess(groundDetected));
        while (gaitController.PeriodicProcess(groundDetected) != RESULT_LEG_IN_TARGET_POSITION) {
            //Do nothing
        }
        leg::ServosPositions expected = leg::ServosPositions();
    ...
    TEST(GaitController, test_Forward_Tripod_init) {
        ...
        ASSERT_NEAR(0.0, gaitController.legs[0].m_controller.GetCoordinates().x.GetCoordinate(), 0.001);

        ASSERT_EQ(RESULT_DIRECTION_NOT_CHOSEN, gaitController.ChangeGait(logic::gait::TRIPOD));
        ASSERT_EQ(RESULT_OK, gaitController.ChangeDirection(logic::GaitController::FORWARD));

        ASSERT_EQ(logic::gait::TRIPOD, gaitController.p_ptr_gaitInterface->GetCurrentGait());
    ...
    TEST(GaitController, test_Foreward_Tripod_walk) {
        ...
        //Check if the leg position controller positions are correct
        ASSERT_NEAR(Constants::DELTA_X_MM, gaitController.legs[0].m_controller.GetCoordinates().x.GetCoordinate_mm(), 0.001);
        ASSERT_NEAR(Constants::DELTA_X_MM, gaitController.legs[1].m_controller.GetCoordinates().x.GetCoordinate_mm(), 0.001);
